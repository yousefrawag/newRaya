const PropertyStauts =  require("../model/PropertyStauts")
exports.addNew = async (req , res , next ) => {
        const {name} = req.body
        if(name){
            const addnew = await PropertyStauts.create({name})
...
exports.getAll = async (req , res , next) => {
    try {
            const allPayments = await PropertyStauts.find({}).sort({ createdAt: -1 })
...
exports.Updateone = async (req , res , next) => {
        const {id} = req.params
        const {name} = req.body
        const updateNew = await PropertyStauts.findByIdAndUpdate(id , {
            name
        } , {new:true})
...
exports.Deleateone = async (req , res , next) => {
        const {id} = req.params
        const currentcurrency = await PropertyStauts.findById(id)
        if(currentcurrency) {
            await PropertyStauts.findByIdAndDelete(id)
